...
| tax | screen | partial | `src/features/tax` — one member's capital gains for a tax year, netted across listed equity, equity funds, gold and unlisted shares: set-off of losses, the ₹1.25 lakh equity allowance, tax at the rates that have their own, short-term gold and unlisted gain handed back as income for the slab, and every sale listed by lot with the specific reason for any it will not net. Not built: the regime comparison and computation table, salary and deductions, slabs, surcharge and cess, foreign shares (the prescribed exchange rate), debt funds (acquisition-date rule), property (indexation election), carried-forward losses, the holding-period clock, advance tax. See Departures |
