...
| Type: mono, labels and units | the hero and stat values in Plex Mono; every label uppercase and letterspaced; `₹5.3 L` as one string | the hero and stat values in Public Sans with `tabular-nums`; sentence-case 12px labels (`.label`); the suffix as a unit span; the figure in a `Qualifier` in the sans face too | `docs/tokens.md` §3 was changed on finding 11 of the UI review, and the mock predates it. Mono is kept for figures in columns and for the table header (`.micro-label`, on a `th` only), so the alignment guarantee stands. A ticker symbol and the sign-in secret are strings, not figures; the symbol is in the sans face and the secret stays mono because it is read character by character. The prototype has not been redrawn; where the two disagree, §3 wins. |
