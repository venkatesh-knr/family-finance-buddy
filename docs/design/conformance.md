...
| Type: mono and units | the hero and stat values in Plex Mono; `₹5.3 L` as one string | the hero and stat values in Public Sans with `tabular-nums`; the suffix as a unit span | `docs/tokens.md` §3 was changed on finding 11 of the UI review, and the mock predates it. Mono is kept for figures in columns and for the table header, so the alignment guarantee stands. The prototype has not been redrawn; where the two disagree, §3 wins. |
