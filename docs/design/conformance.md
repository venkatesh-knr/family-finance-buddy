...
| Currency control | in the avatar menu *and* Settings | Settings alone | Two controls for one setting drift apart, so it landed in one place — the device group, since §366 files display currency with the theme and §368 keeps base currency with the household. It offers the base currency plus whatever the household has a rate for, rather than every ISO code, because the rest would all answer "no rate for that". |
