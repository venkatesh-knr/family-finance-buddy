...
| Editing and ownership on a row | an underlined word ("Correct") and the member's name beside a colour dot | a pencil (`EditButton`) whose accessible name says what it edits, and the member's initial in a ring of their colour (`MemberTag`) | "Correct" repeated down a list is noise and read as though the row were wrong; the name in ink was the loudest thing on the line after the amount. The pencil takes the person to the editor: the caret goes into its first field and it scrolls into view, because an editor that opens elsewhere, with the pressed button still focused, looks as though the press did nothing. The initial is text, so ownership is still not carried by colour alone; where two members share an initial the ones that collide grow to two letters (Ve, Vi), and only where that fails too is the name shown. |
