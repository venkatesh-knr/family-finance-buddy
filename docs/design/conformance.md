...
|---|---|---|---|
| overview | screen | partial | `src/features/overview` — net worth, assets per currency, allocation by kind, assets since inception, rate entry, reading gaps, month close. Not built: the donut, the contributed line, member attribution |
...
| Allocation donut | an SVG donut with a legend | a row per class: colour, name, share, value, return | Taken from the reference app rather than from the mock. A bar or an arc shows one thing — relative size — where a row of the same height shows four, and on a phone that is the difference between a picture and an answer. The colours are the chart palette, but fixed by class (`kindColour` in `src/ui/labels.ts`) and not by rank: coloured by position, Bonds is one colour when it is third largest and another when it is second, and a class must look the same on the donut, in the rows and on Holdings. |
