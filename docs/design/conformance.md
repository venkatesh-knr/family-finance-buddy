...
|---|---|---|---|
| overview | screen | partial | `src/features/overview` — net worth, assets per currency, allocation by kind with its donut, assets since inception, rate entry, reading gaps, month close. Not built: the contributed line, member attribution |
...
| unit | component | built | `src/ui/primitives.tsx` — `Unit` and `Qualifier`, a word beside a figure in the prose face. `docs/tokens.md` §3 keeps mono for figures in a column and the table header; a word placed inside a mono element inherits it, and these undo that. Not the same thing as the magnitude suffix of §3, Units, which is `Amount`. No prototype equivalent |
| amount | component | built | `src/ui/primitives.tsx` — `Amount`, a figure with its magnitude suffix (`L`, `Cr`, `K`) as a `0.6em` muted span welded on, from `formatMoneyParts` in `src/lib/money.ts`. Used on the compact figures in the page; text that cannot hold a span (aria-labels, titles, SVG) keeps the plain string. No prototype equivalent |
...
| Refusing a total | one figure, always | the figure, or the per-currency figures with the reason marked on them | *Resolved as a departure* — `20260908130000` added `fx_rate` and `liability.outstanding_minor`, so the headline is net worth as designed. What is kept is the refusal underneath it: a total that cannot be converted honestly comes back as what each currency holds, marked with which rates are missing, rather than a number short by the dollar holdings. The refusal used to be a red paragraph standing where the figure goes; it is now a caveat on the figure. Net worth leads as the screen's one hero figure (`docs/tokens.md` §3), and when there is no total the hero slot carries the per-currency figures with the refusal marked on them, while the Assets card below carries the same as stat tiles. Assets used to sit above net worth so the screen led with what was known; that answered the red paragraph, which is gone, and it left three figures at the hero step with net worth third. |
| Since-inception chart | net worth as market value against contributed, from the first month | assets only, in one line, from the first month every holding has a reading, in `--ink-2` | Two things the mock could assume and the data cannot. There is no dated debt to subtract, so the line is titled Assets and not Net worth, and the heading says so. And a contributed line needs every purchase and sale dated and in one currency; lots exist, but a holding imported from a statement covers only part of its units (see A partial statement), so a contributed line would be a second figure that is wrong in a way the first is not. Deferred, not dropped. The colour is not `--c2` as in the mock, because a class owns that colour and a total in it reads as equity. Months are converted at each month's own rate, and the line starts where a rate or a reading first exists, with the reason written under it. |
| Allocation donut | an SVG donut with a legend | the donut, one per currency, beside a row per class: colour, name, share, value, return | Taken from the reference app rather than from the mock, and the rows stayed when the donut arrived. A bar or an arc shows one thing — relative size — where a row of the same height shows four, and on a phone that is the difference between a picture and an answer. The ring and the dots read the same fixed colours, from the chart palette, by class (`kindColour` in `src/ui/labels.ts`) and not by rank: coloured by position, Bonds is one colour when it is third largest and another when it is second, and a class must look the same on the donut, in the rows and on Holdings. |
...
| Tax rules | a table a Budget edits | a table only a reviewed migration edits, with the date each row was last checked shown on the page | A household that could edit its own rates could compute the figure it wanted. Nothing fetches the law: the risk is a Budget changing a rate while the app quietly uses last year's, so the date somebody last looked is shown, and a warning appears when it is older than the tax year's Budget. A folded card on the Tax screen (`RatesCard`) lists the bands, rebate, standard deduction, surcharge and cess the figure was rated with, read through the same lookups as the calculation, with where each comes from. The year picker offers only this year and the years that have a sale in them, and is not shown at all when there is only one. |
| Type: mono, labels and units | the hero and stat values in Plex Mono; every label uppercase and letterspaced; `₹5.3 L` as one string | the hero and stat values in Public Sans with `tabular-nums`; sentence-case 12px labels (`.label`); the suffix as a unit span; every figure that is not in a `td` in the sans face with `tabular-nums` (the stat-like `dd`s on Holdings and Tax, list figures, the `Qualifier` figure, the `Delta` chip), so mono is now on table cells, the table header, the sign-in secret and the numeric inputs; the pill row is exempt, as `docs/tokens.md` §3 says | `docs/tokens.md` §3 was changed on finding 11 of the UI review, and the mock predates it. Mono is kept for figures in columns and for the table header (`.micro-label`, on a `th` only), so the alignment guarantee stands. A ticker symbol and the sign-in secret are strings, not figures; the symbol is in the sans face and the secret stays mono because it is read character by character. The prototype has not been redrawn; where the two disagree, §3 wins. |
| Screen width | one measure for every screen | 880px, and 1200px for Overview alone, which is a two-column grid from 1024px | 880px is right for a column with a table or a form in it and wrong for a grid of independent blocks, which on a laptop was a single column beside an empty screen. Only Overview is that; Tax stays at 880px because its blocks are read in order, as a working paper. The columns are placed explicitly so a phone reads the cards in the order they are written. |
| Editing and ownership on a row | an underlined word ("Correct") and the member's name beside a colour dot | a pencil (`EditButton`) whose accessible name says what it edits, and the member's initials in a ring of their colour (`MemberTag`) | "Correct" repeated down a list is noise and read as though the row were wrong; the name in ink was the loudest thing on the line after the amount. The pencil takes the person to the editor: the caret goes into its first field and it scrolls into view, because an editor that opens elsewhere, with the pressed button still focused, looks as though the press did nothing. The initials are text, so ownership is still not carried by colour alone; the mark is the first and last initial as a chat app shows it (VN), a note in brackets is ignored, two that collide fall back to two letters of the first name (Ve, Vi), and only where that fails too is the name shown. |
